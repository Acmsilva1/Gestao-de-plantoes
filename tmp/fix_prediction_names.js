...
async function fix() {
    try {
        console.log('--- Iniciando correção de nomes na tabela dados_predicao ---');
        
        for (const [oldName, info] of Object.entries(mapping)) {
            console.log(`Atualizando: ${oldName} -> ${info.name} (${info.regional})`);
            const { error } = await supabase.from('dados_predicao')
                .update({ unidade: info.name, regional: info.regional })
                .eq('unidade', oldName);
            if (error) console.error(`  - Erro em ${oldName}:`, error.message);
        }

        // Criar dados para unidades novas (opcional, mas bom para o dashboard)
        const newUnits = [
        ...
        for (const u of newUnits) {
            console.log(`Criando dados para unidade nova: ${u.name} (copiando de ${u.copyFrom})`);
            const { data: sourceData } = await supabase.from('dados_predicao').select('*').eq('unidade', u.copyFrom).limit(100);
            if (sourceData && sourceData.length > 0) {
                const newData = sourceData.map(row => {
                    const { id, ...rest } = row;
                    return { ...rest, unidade: u.name, regional: u.regional };
                });
                const { error } = await supabase.from('dados_predicao').insert(newData);
                if (error) console.error(`  - Erro ao criar dados para ${u.name}:`, error.message);
            }
        }
