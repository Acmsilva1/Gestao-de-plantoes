...
- verificacao backend+frontend apos mudancas (`npm run dev`/`npm run dev:full` e `npm run build` na raiz).

## 1. Arquitetura e Estrutura [Status: CONCLUIDO]
- Modularizacao: backend em `backend/` (MVC: `controllers/`, `services/`, `models/`) e frontend em `frontend/`.
...
- [x] Criada migracao SQL: `2026-04-03_fix_fk_cancelamento_escala.sql` (manter cópia em `backend/models/migrations/` quando existir no repositório).
