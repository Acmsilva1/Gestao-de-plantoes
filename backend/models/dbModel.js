...
export const dbModel = {
    ...
    async ensureManagersPerUnit() {
        ...
        if (!masterExists) {
            ...
            const masterRowResp = await supabase
                .from('gestores')
                .select('id, perfil_id')
                .eq('usuario', 'gestor.master')
                .maybeSingle();
            const masterRow = unwrap(masterRowResp, 'Falha ao carregar gestor master');
            if (masterRow?.id && String(masterRow.perfil_id || '') !== String(perfilMaster.id)) {
                const fixResp = await supabase
                    .from('gestores')
                    .update({ perfil_id: perfilMaster.id })
                    .eq('id', masterRow.id);
                unwrap(fixResp, 'Falha ao corrigir perfil do gestor master');
            }
        }

        try {
            const { getCsvStore } = await import('../data/local/db.js');
            await getCsvStore();
        } catch (e) {
            console.warn('[dbModel] Seed sintético local:', e?.message || e);
        }
